...
def get_processed_mnist_data(group_size):
    ...
    zeroes_train = np.array([x_train[i] for i in range(len(x_train)) if y_train[i] == 2])
    ones_train = np.array([x_train[i] for i in range(len(x_train)) if y_train[i] == 7])

    zeroes_test = np.array([x_test[i] for i in range(len(x_test)) if y_test[i] == 2])
    ones_test = np.array([x_test[i] for i in range(len(x_test)) if y_test[i] == 7])

    # The labels are an array of shuffled 0's and 1's
    
    shuffled_labels_train = np.array([2 for i in range(len(zeroes_train))] + [7 for i in range(len(ones_train))])
    shuffled_labels_test = np.array([2 for i in range(len(zeroes_test))] + [7 for i in range(len(ones_test))])
    ...
    #dim_1, dim_2 = 5, 4
    final_data_train, final_labels_train = group(shuffled_data_train, shuffled_labels_train, group_size, 7)
    final_data_test, final_labels_test = group(shuffled_data_test, shuffled_labels_test, group_size, 7)
